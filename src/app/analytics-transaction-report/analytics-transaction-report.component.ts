...
@Component({
  selector: 'app-analytics-transaction-report',
  templateUrl: './analytics-transaction-report.component.html',
  styleUrls: ['./analytics-transaction-report.component.css']
})
export class AnalyticsTransactionReportComponent implements OnInit {
  ...
  getTransactionByChannel(type){
    var req = 'message/getTransactionByChannel/'+type;
    this.commonServiceCall.postResponsePromise(req).subscribe(data => {
      if (data.status) {
        this.transReportCntHeader = [];
        this.transReportCntValue = [];
        this.tableViewData = [];
        data.resp.forEach(el => {
          this.transReportCntValue.push(el[0]);
          this.transReportCntHeader.push(el[1]);
          let Obj = {
            channel : el[1],
            cnt: el[0]
          }
  ...
  getTransactionByChannelNew(param){
    var req = 'message/getTransactionByChannelNew';
  ...
  loadBarChart(){
    ...
    let myChart = new Chart(this.ctxBar, {
      type: 'bar',
      data: {
          labels: this.transReportCntHeader,
          datasets: [{
              label: '# of Votes',
              data: this.transReportCntValue,
              borderWidth: 1
          }]
      },
      options: {
