...
export class CustomerOfferComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.commonServiceCall.pageName = "Offer";
    this.getStatus();
    this.getProductType();
  ...
  gotoAddOffer() {
    this.commonDataService.submenuname = "customerOfferAdd";
    this.router.navigateByUrl("/customerOfferAdd");
  }

  getCustOfferDtl(item) {
    console.log(item);
    if (item.statusName === 'ADMIN APPROVER PENDING' && this.commonDataService.roleType == this.commonDataService.corpMakerRole) {
      showToastMessage('You Cannot Perform This Action');
    }
    else {
      this.commonDataService.submenuname = "customerOfferEdit";
      this.commonServiceCall.makerRequestEditUrl = this.router.url;
      this.commonDataService.offerData.createdon = item.createdon;
      this.router.navigateByUrl("/customerOfferEdit", { state: { id: item.id } });
    }
  }
