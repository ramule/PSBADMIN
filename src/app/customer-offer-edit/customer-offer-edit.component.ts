...
@Component({
  selector: 'app-customer-offer-edit',
  templateUrl: './customer-offer-edit.component.html',
  styleUrls: ['./customer-offer-edit.component.css']
})
export class CustomerOfferEditComponent implements OnInit {
  ...
  constructor(
    ...
    private customerOfferEditService: CustomerOfferEditService
  ) { }
  ...
  addAuditTrailAdaptor(URL, operation) {
    var param = this.customerOfferEditService.addAuditTrailAdaptorParams(URL, operation);
    console.log(param)
  ...
  updateCustomerOfferWithRemark(formdata) {
    ...
    if (this.remarkForm.valid) {
      ...
      var param = this.customerOfferEditService.updateOfferWithRemarkCall(this.custOfferEditFields, this.images, this.userDtls.user_ID, this.userDtl.id, this.remarkForm.value);
  ...
  update() {
    ...
    if (this.customerOfferForm.valid) {
      ...
      console.log('small img: ', this.images.smallImage);
      console.log('large img: ', this.images.largeImage);
      var param = this.customerOfferEditService.updateOfferCall(this.customerOfferForm.value, this.images, this.userDtls.user_ID, this.userDtl.id);
      this.updateOfferMaster(param);
    } else {
      this.formErrors = this.formValidation.validateForm(this.customerOfferForm, this.formErrors, false)
    }
  ...
  getOfferDetails(id) {
    ...
    this.commonServiceCall.getResponsePromise(this.appConstants.getOfferDtlsById + "/" + id).subscribe(data => {
      ...
      if (res.responseCode == "200") {
        ...
        if(result.baseImageSmall == null || result.baseImageSmall == "" || result.baseImageSmall == undefined) {
          this.smallImage = "";
        }
        else {
        ...
        if(result.baseImageLarge == null || result.baseImageLarge == "" || result.baseImageLarge == undefined) {
          this.bigImage = "";
        }
        else {
  ...
  cancel() {
    if (this.commonServiceCall.makerRequestEditUrl == '/customerOffer') {
      this.router.navigateByUrl("/customerOffer");
    }
