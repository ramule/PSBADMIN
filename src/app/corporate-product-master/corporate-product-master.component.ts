...
@Component({
  selector: 'app-corporate-product-master',
  templateUrl: './corporate-product-master.component.html',
  styleUrls: ['./corporate-product-master.component.css']
})
export class CorporateProductMasterComponent implements OnInit {
  id = 1351;
  menuLink = "corporateProductMaster";
  ...
  constructor(
    ...
    private masterProductService: CorporateProductMasterService,
  ...
  ngOnInit() {
    ...
    this.commonServiceCall.pageName = "Corporate Product Master";
  ...
  getAllMasterProductDetails() {
    ...
    this.commonServiceCall.getResponsePromise(this.appConstants.getCorpProducts).subscribe((data) => {
      $('#dt-sample').DataTable().clear().destroy();
      console.log(data);
      var res = data.resp;
      console.log(res);
      if (res.responseCode == "200") {
        this.addAuditTrailAdaptor("Request:" + this.appConstants.apiURL.serviceURL_ESB + this.appConstants.getCorpProducts + "\n" + "Params={}", 'view')
        ...
        this.commonMethod.hideLoader();
        this.errorCallBack(this.appConstants.getCorpProducts, res);
      }
    });
  }
  ...
  showHideForm() {
    this.commonServiceCall.pageName = "Add Corporate Product Master";
    this.showForm = !this.showForm
  ...
  saveMasterProductDetails(param) {
    this.commonMethod.showLoader();
    this.commonServiceCall.postResponsePromise(this.appConstants.saveCorpProductDetails, param).subscribe(data => {
      var res = data.resp;
      if (res.responseCode == "200") {
        this.addAuditTrailAdaptor("Request:" + this.appConstants.apiURL.serviceURL_ESB + this.appConstants.saveCorpProductDetails + "\n" + "Params=" + JSON.stringify(param), 'add')
  ...
  gotoMasterProductDetails(item) {
    ...
    if (item.statusName === 'CORP_APPROVER_PENDING' && this.commonData.roleType == this.commonData.corpMakerRole) {
      ...
      this.router.navigateByUrl("/corporateProductEditMaster", { state: { id: item.id } });
