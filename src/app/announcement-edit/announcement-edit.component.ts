...
export class AnnouncementEditComponent implements OnInit {
  ...
  constructor(
    ...
    public announcementService: AnnouncementEditService
  ) { }
  ...
  public buildForm() {
    this.announcementEditForm = this.form.group({
      ...
      caption: new FormControl('', [Validators.required, Validators.maxLength(20), Validators.pattern(/^(?=.*[a-zA-Z])[a-zA-Z0-9 ]+$/)]),
      announcementHeaderName: new FormControl('', [Validators.required, Validators.pattern(/^(?=.*[a-zA-Z])[a-zA-Z0-9 ]+$/)]),
      productType: new FormControl('', [Validators.required]),
      status: new FormControl('', [Validators.required]),
      seqNo: new FormControl('', [Validators.required, Validators.maxLength(10), Validators.min(1)]),//aplha umeric
    ...
    this.announcementEditForm.valueChanges.subscribe((data) => {
      this.formErrors = this.formValidation.validateForm(this.announcementEditForm, this.formErrors, true)
    });
  ...
  addAuditTrailAdaptor(URL, operation) {
    var param = this.announcementService.addAuditTrailAdaptorParams(URL, operation);
    console.log(param)
  ...
  getAnnouncementDetailsById(id) {
    this.commonMethod.showLoader();
    this.commonServiceCall.getResponsePromise(this.appConstant.getAnnouncementsById + id).subscribe((data) => {
      var res = data.resp;
      console.log(res);
      if (res.responseCode == "200") {
        ...
        if (res.result[0].userAction != null) {
          this.announcementEditForm.patchValue({
            ...
            fromDate: result.validFrom != null ? this.datePipe.transform(result.validFrom, 'yyyy-MM-dd') : '-',
            toDate: result.validTo != null ? this.datePipe.transform(result.validTo, 'yyyy-MM-dd') : '-'
          });
        }
        else {
          this.announcementEditForm.patchValue({
  ...
  getAnnouncementType() {
    this.commonMethod.showLoader();
    this.commonServiceCall.getResponsePromise(this.appConstant.getAnnouncementType).subscribe(data => {
  ...
  filterProducts() {
    return this.productType.filter(x => x.shortName == "MOBILE" || x.shortName == "DESKTOP");
  }
  ...
  openActionModel(action, formdata) {
    ...
    if (this.announcementEditForm.valid) {
      ...
      this.annoucementEditFields.announcementHeaderName = formdata.announcementHeaderName;
      ...
      this.formErrors = this.formValidation.validateForm(this.announcementEditForm, this.formErrors, false)
    }
  }
  ...
  updateCustomerOfferWithRemark(formdata) {
    ...
    if (this.remarkForm.valid) {
      ...
      var param = this.announcementService.updateAnnouncementWithRemarkCall(this.annoucementEditFields, this.images, this.userDtls, this.announcementDetail.id, this.beforeParam, this.remarkForm.value);
  ...
  updateAnnouncement() {
    ...
    this.isValidSmallFileFormat = false;
    this.isValidBigFileFormat = false;
    if (this.announcementEditForm.get('bigImage').value == "") { this.isBigImgError = true; }
    if (this.announcementEditForm.get('smallImage').value == "") { this.isSmallImgError = true; }
    ...
    if (this.announcementEditForm.valid) {
      ...
      var param = this.announcementService.updateAnnouncementCall(this.announcementEditForm.value, this.images, this.userDtls, this.announcementDetail.id, this.beforeParam);
      this.updateAnnouncementDetails(param);
  ...
  cancel() {
    if (this.commonServiceCall.makerRequestEditUrl == '/announcement') {
      this.router.navigateByUrl("/announcement");
    }
  ...
  updateAnnouncementDetails(param) {
    this.commonMethod.showLoader();
    this.commonServiceCall.postResponsePromise(this.appConstant.updateAnnouncementsDetails, param).subscribe(data => {
      console.log(data);
      var res = data.resp;
      if (res.responseCode == "200") {
        this.addAuditTrailAdaptor("Request:" + this.appConstant.apiURL.serviceURL_ESB + this.appConstant.updateAnnouncementsDetails + "\n" + "Params=" + JSON.stringify(param) + "\n" + "Before Update Params=" + JSON.stringify(this.beforeParam), 'update')
