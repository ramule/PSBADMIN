...
@Component({
  ...
  styleUrls: ["./approver-requests.component.css"],
})
export class ApproverRequestsComponent implements OnInit {
  remarkForm: FormGroup;
  formErrors = {
    remark: "",
  };
  approverRequests: any = [];
  ...
  constructor(
    ...
    private approverRequestService: ApproverRequestsService,
  ...
  ngOnInit(): void {
    this.commonServiceCall.pageName = "Approver Requests";
  ...
  getApproverRequests() {
    ...
    this.commonServiceCall
      .getResponsePromise(this.appConstants.getAllDataForApprover)
      .subscribe((data) => {
        var res = data.resp;
        if (res.responseCode == "200") {
          ...
          console.log(res);
          this.approverRequests = res.result;
          this.approverRequests.forEach((el) => {
            el.isApproveChecked = false;
            el.isRejectChecked = false;
          });
          console.log(this.approverRequests);
          if (this.approverRequests.length < 1) {
          ...
          this.errorCallBack(this.appConstants.getAllDataForApprover, res);
        }
  ...
  approveRequest(formdata) {
    if (this.remarkForm.valid) {
      ...
      var param = this.approverRequestService.approveCall(
        this.approveReqItem,
        formData
      );
  ...
  rejectRequest(formdata) {
    if (this.remarkForm.valid) {
      closeTinyModel1();
      var formData = this.remarkForm.value;
      var param = this.approverRequestService.rejectCall(
        this.rejectReqItem,
        formData
      );
      this.approveRejectRequest(param);
  ...
  approveRejectRequest(param) {
    this.commonMethod.showLoader();
    this.commonServiceCall
      .postResponsePromise(this.appConstants.approverReqUrl, param)
      .subscribe((data) => {
        var res = data.resp;
        if (res.responseCode == "200") {
          ...
          this.errorCallBack(this.appConstants.approverReqUrl, res);
        }
        this.commonMethod.hideLoader();
      });
  }
    ...
    switch (type) {
      case "approve":
        if ($("#approveCheckBox").is(":checked")) {
          this.approverRequests.map((v) => (v.isApproveChecked = true));
          $("input.rejectall").attr("disabled", true);
          $("#rejectCheckBox").attr("disabled", true);
          this.actionButton = "Approve All";
          $(':input[type="submit"]').prop("disabled", false);
        } else {
          this.approverRequests.map((v) => (v.isApproveChecked = false));
      ...
      case "reject":
        if ($("#rejectCheckBox").is(":checked")) {
          this.approverRequests.map((v) => (v.isRejectChecked = true));
          $("input.approveall").attr("disabled", true);
          $("#approveCheckBox").attr("disabled", true);
          this.actionButton = "Reject All";
          $(':input[type="submit"]').prop("disabled", false);
        } else {
          this.approverRequests.map((v) => (v.isRejectChecked = false));
  ...
  getdata() {
    ...
    if (this.actionButton == "Approve All") {
      var newarray = this.approverRequests;
  ...
  selectsingle(type, item) {
    ...
    switch (type) {
      case "approve":
        var objIndex = this.approverRequests.findIndex(
          (obj) => obj.id == item.id
        );
        if (this.approverRequests[objIndex].isApproveChecked == true) {
          this.approverRequests[objIndex].isApproveChecked = false;
        } else {
          this.approverRequests[objIndex].isApproveChecked = true;
        }
        if (this.approverRequests.find((el) => el.isApproveChecked == true)) {
          $("input.rejectall").attr("disabled", true);
          $("#rejectCheckBox").attr("disabled", true);
          this.actionButton = "Approve All";
          $(':input[type="submit"]').prop("disabled", false);
      ...
      case "reject":
        ...
        if (this.approverRequests[objIndex].isRejectChecked == true) {
          this.approverRequests[objIndex].isRejectChecked = false;
        } else {
          this.approverRequests[objIndex].isRejectChecked = true;
        }
        if (this.approverRequests.find((el) => el.isRejectChecked == true)) {
          $("input.approveall").attr("disabled", true);
          $("#approveCheckBox").attr("disabled", true);
          this.actionButton = "Reject All";
          $(':input[type="submit"]').prop("disabled", false);
  ...
  approveall(formData) {
    ...
    if (this.remarkForm.valid) {
      ...
      this.commonServiceCall
        .postResponsePromise(
          this.appConstants.updateStatusListByApprover,
          this.finalarray
        )
        .subscribe((data) => {
          var res = data.resp;
