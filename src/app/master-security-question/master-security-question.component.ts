...
@Component({
  selector: 'app-master-security-question',
  templateUrl: './master-security-question.component.html',
  styleUrls: ['./master-security-question.component.css']
})
export class MasterSecurityQuestionComponent implements OnInit {
  ...
  constructor(
    ...
    private masterSecurityQuestionService: MasterSecurityQuestionService,
  ...
  public buildForm() {
    ...
    this.masterSecurityQuestionForm.valueChanges.subscribe((data) => {
      this.formErrors = this.formValidation.validateForm(this.masterSecurityQuestionForm, this.formErrors, true)
    });
  ...
  ngOnInit() {
    ...
    this.commonServiceCall.pageName = "Security Question";
...
    }
  ...
  getAllSecurityQuestionDetails() {
    this.securityQuesArr = [];
    this.commonMethod.showLoader();
    this.commonServiceCall.getResponsePromise(this.appConstants.getSecurityQuestions).subscribe((data) => {
      $('#dt-sample').DataTable().clear().destroy();
      var res = data.resp;
      console.log(res);
      if (res.responseCode == "200") {
           this.addAuditTrailAdaptor("Request:"+this.appConstants.apiURL.serviceURL_ESB+this.appConstants.getSecurityQuestions+"\n"+"Params={}",'view')
        this.commonMethod.hideLoader();
        console.log('response data: ', res);
        ...
        res.result.forEach(element => {
          if(element.appname != "CORPORATE") {
            this.securityQuesArr.push(element);
          }
        });
      } else {
        setTimeout(function () {
  ...
  addSecurityQuestion() {
    ...
    if (this.masterSecurityQuestionForm.valid) {
      var param = this.masterSecurityQuestionService.addSecurityQuestionCall(this.masterSecurityQuestionForm.value);
      console.log('request parameters: ', param);
      this.saveMasterSecurityQuestionDetails(param);
    } else {
      this.formErrors = this.formValidation.validateForm(this.masterSecurityQuestionForm, this.formErrors, false)
    }
  }

  saveMasterSecurityQuestionDetails(param) {
    this.commonMethod.showLoader();
    this.commonServiceCall.postResponsePromise(this.appConstants.addSecurityQuestions, param).subscribe(data => {
      var res = data.resp;
      if(res.responseCode == "200"){
           this.addAuditTrailAdaptor("Request:"+this.appConstants.apiURL.serviceURL_ESB+this.appConstants.addSecurityQuestions+"\n"+"Params="+JSON.stringify(param),'add')
  ...
  showHideForm() {
    this.commonServiceCall.pageName = "Security Question Add"
    this.showForm = !this.showForm
  ...
  gotoSecurityQuestionDetails(item) {
    ...
    if(item.statusname === 'ADMIN APPROVER PENDING' && this.commonData.roleType == this.commonData.makerRole) {
      ...
      this.router.navigateByUrl("/securityQuestionEdit",{ state: { id: item.id} });
    }
  }
  ...
  addSecurityQuestionWithRemark(formdata){
    ...
    if (this.remarkForm.valid) {
      ...
      var param = this.masterSecurityQuestionService.addSecurityQuestionCallWithRemark(this.securityQuestionFields, formData);
      this.saveMasterSecurityQuestionDetails(param);
    } else {
