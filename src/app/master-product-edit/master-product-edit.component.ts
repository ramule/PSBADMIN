...
@Component({
  ...
  templateUrl: "./master-product-edit.component.html",
  styleUrls: ["./master-product-edit.component.css"],
})
export class MasterProductEditComponent implements OnInit {
  ...
  constructor(
    ...
    private masterProductService: MasterProductEditService,
  ...
  ngOnInit() {
    ...
    if(this.commonData.browserRefresh) {
      this.buildForm();
      this.router.navigateByUrl('/masterProduct');
      return;
    }

    this.commonServiceCall.pageName = "Edit Product Master";
  ...
  cancel() {
    if (this.commonServiceCall.makerRequestEditUrl == "/masterProduct") {
      this.router.navigateByUrl("/masterProduct");
  ...
  getProductById(id) {
    ...
    this.commonMethod.showLoader();
    var reqUrl = this.appConstants.getProductById + id;
  ...
  updateMasterProductDetails(param) {
    this.commonMethod.showLoader();
    this.commonServiceCall
      .postResponsePromise(this.appConstants.updateProductDetails, param)
      .subscribe((data) => {
        var res = data.resp;
        if (res.responseCode == "200") {
          this.addAuditTrailAdaptor(
            "Request:" +
              ...
              this.appConstants.updateProductDetails +
              "\n" +
              "Params=" +
          ...
          if(this.commonData.roleType == this.commonData.makerRole) {
            this.masterProductEditForm.patchValue({
              ...
              appId: this.masterProductFields.appId,
            });
          }
          showToastMessage(res.responseMessage);
        }
  ...
  openActionModel(action, formdata) {
    if (this.masterProductEditForm.valid) {
      ...
      this.masterProductFields.appId = formdata.appId;
    } else {
      this.formErrors = this.formValidation.validateForm(
        this.masterProductEditForm,
        this.formErrors,
