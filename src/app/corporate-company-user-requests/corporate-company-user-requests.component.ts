...
declare var $: any;
@Component({
  selector: 'app-corporate-company-user-requests',
  templateUrl: './corporate-company-user-requests.component.html',
  styleUrls: ['./corporate-company-user-requests.component.css']
})
export class CorporateCompanyUserRequestsComponent implements OnInit {
  ...
  constructor(
    ...
    private corporateCompanyUserRequestsService: CorporateCompanyUserRequestsService,
    public commonDataService: CommonDataShareService
  ) { }

  ngOnInit(): void {
    this.commonServiceCall.pageName = "Corporate Company Offline Registration";
    this.buildForm();
    this.getLeftMenuId();
  ...
  addAuditTrailAdaptor(URL, operation) {
    var param = this.corporateCompanyUserRequestsService.addAuditTrailAdaptorParams(URL, operation);
    console.log(param)
  ...
  getCorpCompanyDetails() {
    ...
    if (this.corpCompanyForm.valid) {
      ...
      if (this.type == 'rrn') {
        var inputdata = this.corporateCompanyUserRequestsService.getCorpCompanyDetailsByType(formData);
  ...
  getCorpCompanyDetailsByCIFCompnameCompcode(params) {
    ...
    this.commonServiceCall.postResponsePromise(this.appConstants.getOfflineCorpByCompNameCifCorpIdUrl, params).subscribe((data) => {
      var res = data.resp;
      if (res.responseCode == "200") {
        console.log(res);
        //initiallize datatable
        this.addAuditTrailAdaptor("Request:" + this.appConstants.apiURL.serviceURL_ESB + this.appConstants.getOfflineCorpByCompNameCifCorpIdUrl + "\n" + "Params=" + JSON.stringify(params), 'view')
        ...
        this.commonMethod.hideLoader();
        this.errorCallBack(this.appConstants.getOfflineCorpByCompNameCifCorpIdUrl, res);
      }
    });
  }
  ...
  getAllCorpCompanyDetails() {
    $('#dt-sample').DataTable().clear().destroy();
    this.commonMethod.showLoader();
    this.commonServiceCall.getResponsePromise(this.appConstants.getOfflineCorpCompDataUrl).subscribe((data) => {
      var res = data.resp;
      if (res.responseCode == "200") {
        console.log(res);
        this.addAuditTrailAdaptor("Request:" + this.appConstants.apiURL.serviceURL_ESB + this.appConstants.getOfflineCorpCompDataUrl + "\n" + "Params={}", 'view')
        //initiallize datatable
        this.commonMethod.setDataTable(this.commonServiceCall.pageName);
        this.corpCompanyReqArr = res.result;
        this.commonMethod.hideLoader();
        ...
        this.commonMethod.hideLoader();
        this.errorCallBack(this.appConstants.getOfflineCorpCompDataUrl, res);
      }
    });
  }
  ...
  getDtlByType(param) {
    $('#dt-sample').DataTable().clear().destroy();
    this.commonMethod.showLoader();
    this.commonServiceCall.postResponsePromise(this.appConstants.getOfflineCorpCompDataByRrnUrl, param).subscribe((data) => {
      var res = data.resp;
      if (res.responseCode == "200") {
        console.log(res);
        //initiallize datatable
        this.addAuditTrailAdaptor("Request:" + this.appConstants.apiURL.serviceURL_ESB + this.appConstants.getOfflineCorpCompDataByRrnUrl + "\n" + "Params=" + JSON.stringify(param), 'view')
        this.commonMethod.setDataTable(this.commonServiceCall.pageName);
        this.corpCompanyReqArr = res.result;
        this.commonMethod.hideLoader();
      } else {
        ...
        this.commonMethod.hideLoader();
        this.errorCallBack(this.appConstants.getOfflineCorpCompDataByRrnUrl, res);
      }
    });
  }
  ...
  gotoCorpCompanyDetails(item) {
    ...
    if (item.statusname === 'CORP_APPROVER_PENDING' && this.commonDataService.roleType == this.commonDataService.corpMakerRole) {
      ...
      this.commonDataService.submenuname = 'corpCompanyUserRequestsEdit';
      this.commonDataService.customerAgent.createdon = item.createdon;
      this.commonServiceCall.makerRequestEditUrl = this.router.url;
      this.router.navigateByUrl("/corpCompanyUserRequestsEdit", { state: { id: item.id } });
    }
  }

  gtoCorpcompanyUserAdd() {
    this.router.navigateByUrl('/corpCompanyUserRequestsAdd');
    this.commonDataService.submenuname = "corpCompanyUserRequestsAdd";
  }
