...
@Component({
  selector: 'app-activity-response-time',
  templateUrl: './activity-response-time.component.html',
  styleUrls: ['./activity-response-time.component.css']
})
export class ActivityResponseTimeComponent implements OnInit {
  adminActivities: any = [];
  activityLog: any = [];
  tabselected: any;
  selectedDate: any = {};
  ...
  ngOnInit(): void {
    this.commonServiceCall.pageName = "Activity Response Time";
  ...
  getActivityHourlyLogs(params) {
    ...
    this.commonServiceCall.postResponsePromise(this.appConstants.getActivityResponseTime, params).subscribe((data) => {
      var res = data.resp;
      if (res.responseCode == "200") {
        ...
        if (this.activityLogMaster.length >0) {
          ...
          this.colorLabel.forEach(element => {
            ...
            this.chartDataSet.push({label,data,backgroundColor,hoverBorderWidth})
        ...
        tooltips: {
          callbacks: {
            label: function(t, d) {
              ...
              ' : (Hours: ' + t.yLabel + ', Response Time: ' + d.datasets[t.datasetIndex].data[t.index].r+ ')';
            }
          }
        }
      },
  ...
  getGraph()
  {
    this.myChartBar.options.title.text = "Activity Response Time Chart";
    this.myChartBar.update();
